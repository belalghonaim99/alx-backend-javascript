  ...
  if (dp) {
    fs.readFile(dp, (error, data) => {
      ...
      if (data) {
        ...
        const totalOfStudents = Object.values(sg).reduce(
    ...
    handler(_, resposne) {
      ...
      countStudents(DB_FILE)
        .then((report) => {
          ...
          resposne.setHeader('Content-Type', 'text/plain');
          resposne.setHeader('Content-Length', responseText.length);
          resposne.statusCode = 200;
          resposne.write(Buffer.from(responseText));
        })
        .catch((err) => {
